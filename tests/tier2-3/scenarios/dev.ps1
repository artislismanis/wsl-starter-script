...
& "$PSScriptRoot/../run-scenario.ps1" `
  ...
  -InstallSteps @(
    ...
    @{ User='tester'; Command='cd ~/wsl-starter-script && MISE_LANGUAGES=node,python MISE_TOOLS=lazygit,delta,zellij,terraform,databricks,git-spice ZSH_DEFAULT_SHELL=0 ./install.sh --dev --non-interactive' }
    @{ User='tester'; Command='cd ~/wsl-starter-script && MISE_LANGUAGES=node,python MISE_TOOLS=lazygit,delta,zellij,terraform,databricks,git-spice ZSH_DEFAULT_SHELL=0 ./install.sh --dev --non-interactive' }
  ) `
